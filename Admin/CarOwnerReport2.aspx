<%@ Page Title=" ﬁ—Ì— «·„«·ﬂÌ‰" Language="C#" MasterPageFile="Reports.master" %>
...
<asp:Content ContentPlaceHolderID="Header" ID="C1" runat="server">
    <table width="100%">
        ...
        <tr>
            <td colspan="2" align="center">
                ﬂ‘› Õ—ﬂ«  «·—«” «·ﬁ«ÿ— ·‘—ﬂ… <%=DataBase.GetPName("Owners","name","id",Session["owner"].ToString()) %>
            </td>
        </tr>
        <tr>
            <td colspan="2" align="center"> „‰ÿﬁ… «· Õ„Ì· &nbsp;&nbsp;&nbsp;<%=DataBase.GetPName("Centers","name","id",Session["from"].ToString()) %>&nbsp;&nbsp;&nbsp; „‰ÿﬁ… «· ›—Ì€ &nbsp;&nbsp;&nbsp;<%=DataBase.GetPName("Centers","name","id",Session["to"].ToString()) %></td>
        </tr>
    </table>
...
<asp:Content ID="Content1" ContentPlaceHolderID="Body" Runat="Server">
    ...
    <asp:GridView  ID="GridView1" runat="server" AutoGenerateColumns="False" 
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="„‰" SortExpression="FromId">
                <ItemTemplate>
                    <asp:Label ID="Label1" runat="server" Text='<%#Eval("FromId") %>'></asp:Label>
                </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="«·Ï" SortExpression="ToId">
                <ItemTemplate>
                    <asp:Label ID="Label2" runat="server" Text='<%#Eval("ToId") %>'></asp:Label>
                </ItemTemplate>
            </asp:TemplateField>
            <asp:BoundField DataField="Driver_Name" HeaderText="«”„ «·”«∆ﬁ" 
                SortExpression="Driver_Name" />
            <asp:BoundField DataField="Car_Number" HeaderText="—ﬁ„ «·ﬁ«ÿ—…" 
                SortExpression="Car_Number" />
            <asp:BoundField DataField="Trailer_Number" HeaderText="—ﬁ„ «·„ﬁÿÊ—…" 
                SortExpression="Trailer_Number" />
            <asp:BoundField DataField="LeavingTime1" 
                HeaderText=" «—ÌŒ «· Õ„Ì·" DataFormatString="{0:dd/MM/yyyy}" SortExpression="LeavingTime1" />
                
            <asp:BoundField DataField="ArrivalTime1" 
                HeaderText=" «—ÌŒ «· ›—Ì€" DataFormatString="{0:dd/MM/yyyy}" SortExpression="ArrivalTime1" />
            
            <asp:BoundField DataField="LoadNo" 
                HeaderText="—ﬁ„ «· Õ„Ì·" SortExpression="LoadNo" /> 
            
            <asp:BoundField DataField="EmptyingNo" 
                HeaderText="—ﬁ„ «· ›—Ì€" SortExpression="EmptyingNo" />
            
            <asp:BoundField DataFormatString="{0:###,###,###}" DataField="Tonnage" 
                HeaderText="«·Õ„Ê·…" SortExpression="Tonnage" />  
             
           <asp:BoundField DataFormatString="{0:###,###,###}" DataField="Tonnage2" 
                HeaderText="Ê“‰ «· ›—Ì€" SortExpression="Tonnage2" />   
                
           <asp:TemplateField HeaderText="«·Ê“‰ «·„’—Ê›">
                <ItemTemplate>
    ...
    <table class="tr1" width="98%">
        <tr>
            <td>
                ⁄œœ «·‰ﬁ·«  : <%=GridView1.Rows.Count %>
            </td>
            
            <td>
                „Ã„Ê⁄ «Ê“«‰ «· Õ„Ì· : <%=GetSum(10).ToString("###,###,###") %>
            </td>
            
            <td>
                „Ã„Ê⁄ «Ê“«‰ «· ›—Ì€ : <%=GetSum(11).ToString("###,###,###")%>
            </td>
            <td>
                „Ã„Ê⁄ «·Ê“‰ «·„’—Ê› : <%=GetSum(12,"r").ToString("###,###,###")%>
            </td>
        </tr>
    </table>
