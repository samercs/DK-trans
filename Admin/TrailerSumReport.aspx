...
<asp:Content ID="Content3" ContentPlaceHolderID="Body" Runat="Server">
    <asp:SqlDataSource ID="SqlDataSource2" runat="server" 
        ConnectionString="<%$ ConnectionStrings:CS %>" 
        SelectCommand="SELECT Transports.TrailerID, COUNT(*) AS Move_Count,sum(Tonnage) as Tonnage_Sum,sum(Tonnage2) as Tonnage2_Sum,sum(Tonnage-Tonnage2) as Net_Sum FROM Cars INNER JOIN Transports ON Cars.id = Transports.CarID INNER JOIN Trailer ON Transports.TrailerID = Trailer.id where (DATEDIFF(mi,LeavingTime1,@d1)<=0 and DATEDIFF(mi,LeavingTime1,@d2)>=0 and Transports.ToId=@ToId) GROUP BY  Transports.TrailerID">
    ...
    <table class="tr1" width="90%">
        <tr>
            <td>
                عدد النقلات : <%=GetSum(1).ToString("###,###,###") %>
            </td>
            
            <td>
                مجموع اوزان التحميل : <%=GetSum(2).ToString("###,###,###")%>
            </td>
            
            <td>
                مجموع اوزان التفريغ : <%=GetSum(3).ToString("###,###,###")%>
            </td>
            <td>
                مجموع الفروق : <%=GetSum(4).ToString("###,###,###")%>
            </td>
        </tr>
    </table>
</asp:Content>
